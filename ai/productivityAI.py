...
def build_messages(
    ...
    if schedules:
        schedules_text = "现有日程: " + ", ".join(
            f"{sc.schedule_type}:{sc.title}({sc.scheduled_time.strftime('%m月%d日') if sc.scheduled_time else '无期'})" for sc in schedules
        )
    else:
        schedules_text = "暂无日程"
...
调整原则：当用户直接表达身体或情绪的强烈变化（如“发烧”“崩溃”“狂喜”）时，立即将对应维度拉到与表达一致的数值（范围 0-100，可跨越 40+），不要平滑过渡。日常杂谈微调 1-3 点。psychological_harmony_index（PHI）由活力/情绪/关系/自我/意义的理想点(80/75/80/85/75)的均方根偏差计算，偏离即扣分(无论过高过低)，高分不代表高和谐；例如 90,80,95,99,90 → PHI≈35。
...
4. follow_up_text：**仅限**对改名/日程/情绪转折的确认文字（如 已记下新昵称：xxx。 已记录新日程：类型（时间）：内容 等），或一张表格维度变化≥8时的一句简短的客观事实陈述（**必须是一个完整句，不得包含任何问句、建议、关心、追问、开启新话题等**）。当且仅当需要用户确认编辑/删除日程时，可以包含一个简短问句（如“需要我删除‘散步’这个日程吗？”），但不能展开对话。你的回话内容会被接到小元回复的后面，请确保你的语气像是小元说的而非系统。
