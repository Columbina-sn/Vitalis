...
(function() {
    ...
    // ================================================================
    //  1.3 标记引导完成 (POST /user/mark-intro)
    // ================================================================
    ...
    // ================================================================
    //  1.4 获取用户信息 (GET /user/information)
    // ================================================================
    ...
    // ================================================================
    //  1.6 获取五维状态 (GET /user/status) 与渲染
    // ================================================================
    ...
    // ================================================================
    //  1.7 状态历史图表 (GET /user/status-history/{dimension})
    // ================================================================
    ...
    // ================================================================
    //  1.9 修改密码 (POST /user/change-password)
    // ================================================================
    ...
    // ================================================================
    //  1.10 注销账户 (POST /user/delete-account)
    // ================================================================
    ...
    function createRotatingThinkingMessage() {
        ...
        function stopRotation() {
            if (stateTimer) { clearTimeout(stateTimer); clearInterval(stateTimer); stateTimer = null; }
            if (tutorialTimer) { clearTimeout(tutorialTimer); clearInterval(tutorialTimer); tutorialTimer = null; }
        }
        tempDiv.addEventListener('click', () => {
            stopRotation();
            updateState();
            updateTutorial();
            stateTimer = setTimeout(() => { updateState(); stateTimer = setInterval(updateState, 5000); }, 6000);
            tutorialTimer = setTimeout(() => { updateTutorial(); tutorialTimer = setInterval(updateTutorial, 10000); }, 6000);
        });
    ...
    function isToday(date) { const today = new Date(); return isSameDay(date, today); }
    ...
    function resetTimeTracking() { lastRenderedMessageTime = null; }

    function renderMessagesWithDividers(messages, isReset = false, hasEarlier = false) {
        if (!messages.length) return;
        if (isReset) resetTimeTracking();
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (prevTime) {
                if (!isSameDay(prevTime, currentTime)) fragment.appendChild(createLongDivider(currentTime));
                if (shouldShowShortDivider(prevTime, currentTime)) fragment.appendChild(createShortDivider(currentTime));
            } else {
                if (isReset && hasEarlier) { /* 不插入日期，等后续加载 */ } else { fragment.appendChild(createLongDivider(currentTime)); }
            }
        ...
        if (messages.length > 0) {
            lastRenderedMessageTime = new Date(messages[messages.length - 1].created_at);
        }
        if (isReset) {
    ...
    // ================================================================
    //  2.3 日期查询历史 (GET /chat/history/date)
    // ================================================================
    ...
    function bindDateInputFallback() {
        ...
        dateJumpInput.addEventListener('blur', function() {
            if (!this.value) this.value = getTodayDateString();
        });
    }

    function setDefaultDate() {
        if (!dateJumpInput) return;
        dateJumpInput.value = getTodayDateString();
    }
    ...
    function renderDateHistoryMessages(messages, dateStr) {
        ...
        const date = new Date(dateStr);
        dividerSpan.textContent = `${date.getFullYear()}年${date.getMonth() + 1}月${date.getDate()}日`;
    ...
    function createScrollButtons() {
        ...
        function updateBottomBtnPosition() {
            ...
            bottomBtn.style.bottom = `${inputHeight + 12}px`;
        }
        const inputResizeObserver = new ResizeObserver(() => updateBottomBtnPosition());
        ...
        function updateScrollButtons() {
            ...
            if (scrollTop <= 8) topBtn.classList.remove('show');
            else topBtn.classList.add('show');

            if (scrollTop + clientHeight >= scrollHeight - 8) bottomBtn.classList.remove('show');
            else bottomBtn.classList.add('show');
        }
    ...
    function bindChatEvents() {
        ...
        if (dateJumpConfirmBtn) dateJumpConfirmBtn.addEventListener('click', fetchAndShowDateHistory);
        if (closeDateHistoryModalBtn) closeDateHistoryModalBtn.addEventListener('click', closeDateHistoryModal);
        if (dateHistoryModal) dateHistoryModal.addEventListener('click', (e) => { if (e.target === dateHistoryModal) closeDateHistoryModal(); });
        if (dateJumpInput) {
            dateJumpInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') fetchAndShowDateHistory();
            });
        }
